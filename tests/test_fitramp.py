...
def test_basic():
    ...
    y = getramps(countrate, sig, readtimes, nramps=nreads_cmatrix)
    d = y * 1.0
    d[0] /= C.mean_t[0]
    ...
    print(
        "Calculated:",
        (countrate * C.alpha_phnoise + sig**2 * C.alpha_readnoise).round(2),
    )
    ...
    print(
        "Calculated:", (countrate * C.beta_phnoise + sig**2 * C.beta_readnoise).round(2)
    )
    ...
    y = getramps(countrate, sig, readtimes, nramps=nramps_test)
    d = y * 1.0
    d[0] /= C.mean_t[0]
    d[1:] = (y[1:] - y[:-1]) / C.delta_t[:, np.newaxis]
    ...
    for i in range(d.shape[0]):
        ...
        if i < d.shape[0] - 1:
            M[i, i + 1] = M[i + 1, i] = (
                C.beta_readnoise[i] * sig**2 + C.beta_phnoise[i] * ct
            )
    ...
    par, cov = optimize.curve_fit(
        lin, np.arange(d.shape[0]), d[:, 0], sigma=M, absolute_sigma=True
    )
    par_ramp = np.array([r.countrate[0], r.pedestal[0]])
    cov_ramp = np.array(
        [
            [r.uncert[0] ** 2, r.covar_countrate_pedestal[0]],
    ...
    r = fitramp.fit_ramps(
        d, C, sig, countrateguess=np.mean(d[1:], axis=0), diffs2use=diffs2use
    )

    par, cov = optimize.curve_fit(
        lin, np.arange(dp.shape[0]), dp[:, 0], sigma=Mp, absolute_sigma=True
    )
    par_ramp = np.array([r.countrate[0], r.pedestal[0]])
    cov_ramp = np.array(
        [
    ...
    for i in range(nramps_test):
        ...
        chisq = np.linalg.multi_dot(
            [(d[:, i] - slope)[np.newaxis, :], Cinv, d[:, i] - slope]
        )[0]

        if (
            np.abs(slope - result.countrate[i]) > 1e-8
            or np.abs(chisq - result.chisq[i]) > 1e-8
            or np.abs(np.sum(Cinv) ** -0.5 - result.uncert[i]) > 1e-8
        ):
    ...
    for i in range(d.shape[0]):
        ...
        chisq = np.linalg.multi_dot(
            [(d[:, 0] - slope)[np.newaxis, :], Cinv, d[:, 0] - slope]
        )[0]

        if (
            np.abs(slope - result.countrate_oneomit[i, 0]) > 1e-8
            or np.abs(chisq - result.chisq_oneomit[i, 0]) > 1e-8
            or np.abs(np.sum(Cinv) ** -0.5 - result.uncert_oneomit[i, 0]) > 1e-8
        ):
            allpassed = False
    ...
    for i in range(d.shape[0] - 1):
        ...
        if (
            np.abs(slope - result.countrate_twoomit[i, 0]) > 1e-8
            or np.abs(chisq - result.chisq_twoomit[i, 0]) > 1e-8
            or np.abs(np.sum(Cinv) ** -0.5 - result.uncert_twoomit[i, 0]) > 1e-8
        ):
            allpassed = False
    ...
    if (
        np.abs(slope - result.countrate[0]) > 1e-8
        or np.abs(chisq - result.chisq[0]) > 1e-8
        or np.abs(np.sum(Cinv) ** -0.5 - result.uncert[0]) > 1e-8
    ):
        allpassed = False
        print("Failed test for three omitted resultant differences")

    Covar_saved_threeomit = Covar * 1.0

    for i in range(d.shape[0]):
        Covar = Covar_saved_threeomit * 1.0
    ...
    y = getramps(countrate, sig, readtimes, nramps=nramps_testjumps)
    d = (y[1:] - y[:-1]) / C.delta_t[:, np.newaxis]
    ...
    diffs2use, c = fitramp.mask_jumps(
        d, C, sig_arr, threshold_oneomit=18, threshold_twoomit=21
    )
    ...
    if np.abs(numberflagged / factor_expected / numbercaught - 1) > 0.1:
        print(
            "Jump masking flagged an unexpectedly large number of valid resultant differences"
        )
        allpassed = False
